---
layout: term
title: bölme
slug: bolme
letter: B
lisan: Türkçe
anlamlar:
- Bölmek işi
- Salon, oda, sofa vb. büyük bir yerden ayrılmış daha küçük yer
- Büyük bir yeri, alanı küçük oda veya kısımlara ayıran ince duvar veya tahta perde
- Kalın ağaç gövdesinden odun veya tekne yapmak için ayrılan tomruk
- Gemilerin içinde, su baskını, yangın vb. durumlarda, ara kapılar kapandığında arızanın veya hasarın yayılmasını önlemek için kullanılan birbirlerinden ayrılmış yerler
- Cins kavramlarını tür, alt tür kavramlarına ayırma işi
- Bir sayının içinde başka bir sayının kaç defa bulunduğunu belirlemek için başvurulan işlem
ozellikler:
- - isim
- - isim
  - ''
- - isim
  - ''
  - ''
- - isim
  - ''
  - ''
  - ağızlardan
- - isim
  - ''
  - ''
  - ağızlardan
  - denizcilik
- - isim
  - ''
  - ''
  - ağızlardan
  - denizcilik
  - mantık
- - isim
  - ''
  - ''
  - ağızlardan
  - denizcilik
  - mantık
  - matematik
ornekler:
- - ''
- - Sedirin olduğu bölmeyi basma bir perdeyle odadan ayırdı.
- - Ortadan ayrılan bir bölmeyle de diğer nısfında Bora Reis yatıyordu.
- - ''
- - ''
- - ''
- - Bu bakımdan toplama işleminin düşünüldüğü her yerde bir bölme işleminden de söz etmek gerekir.
orneklerkaynak:
- - ''
- - Lâtife Tekin
- - Nâzım Hikmet
- - ''
- - ''
- - ''
- - Utku Özmakas
---
