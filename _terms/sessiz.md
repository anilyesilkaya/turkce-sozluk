---
layout: term
title: sessiz
slug: sessiz
letter: S
lisan: Türkçe
anlamlar:
- Sesi olmayan, ses çıkarmayan
- Ses, gürültü çıkarmadan yapılan
- Ses olmayan
- Az konuşan
- Yumuşak huylu, kendi hâlinde (kimse); sakin
- Ses ve gürültü çıkarmadan
- ► ünsüz
ozellikler:
- - sıfat
- - sıfat
  - ''
- - sıfat
  - ''
  - ''
- - sıfat
  - ''
  - ''
  - ''
- - sıfat
  - ''
  - ''
  - ''
  - ''
- - sıfat
  - ''
  - ''
  - ''
  - ''
  - zarf
- - sıfat
  - ''
  - ''
  - ''
  - ''
  - zarf
  - isim
  - dil bilgisi
ornekler:
- - Işık bol, sofra açık, kadehler pırıl pırıl / Bak, sessiz adımlarla yaklaşıyor yeni yıl / Omzuma koy başını bir gül hafifliğiyle
- - Sessiz çalışma.
- - Gece başladığı vakit, ışıksız ve sessiz evler arasından geçmek icap ediyordu.
- - ''
- - Kız kardeşi Deniz Yolları levazımında çalışan sessiz bir adamla evlidir.
- - Badem gözlüdürler, saçları salkım salkım / Omuzlarındadır ve yaptıkları işi sessiz yaparlar
- - ''
orneklerkaynak:
- - Halit Fahri Ozansoy
- - ''
- - Hamdullah Suphi Tanrıöver
- - ''
- - Memduh Şevket Esendal
- - Ahmet Muhip Dıranas
- - ''
---
