---
layout: term
title: motor
slug: motor
letter: M
lisan: Fransızca moteur
anlamlar:
- Herhangi bir enerjiyi mekanik enerjiye dönüştüren düzenek
- Akaryakıtla işleyen deniz aracı
- motosiklet
- Sinema ve televizyon filmleri çekimi sırasında yönetmenin “başla” anlamında verdiği komut
- Kendisini cinsel bakımdan kullandıran kimse
ozellikler:
- - isim
- - isim
  - ''
- - isim
  - ''
  - ağızlardan
- - isim
  - ''
  - ağızlardan
  - sinema
  - televizyon
- - isim
  - ''
  - ağızlardan
  - sinema
  - televizyon
  - argo
ornekler:
- - Taşındıkları kamyon önünde durunca motorun patırtısı kesildi.
- - Rıhtıma varabildiği zaman vapura gidecek olan son motor kalkmak üzere idi.
- - ''
- - ''
- - Para için her erkeğin altına yatacak piyasa motorlarına o şahane roman kahramanlarının adını veriyorlar.
orneklerkaynak:
- - Necati Cumalı
- - Falih Rıfkı Atay
- - ''
- - ''
- - Ahmet Ümit
---
