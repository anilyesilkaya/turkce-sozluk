---
layout: term
title: adres
slug: adres
letter: A
lisan: Fransızca adresse
anlamlar:
- Herhangi bir kişi veya mekânın bulunduğu şehir, ilçe, mahalle, cadde, sokak, kapı veya daire numarasının belirtildiği bilgi; bulunak
- Herhangi bir kişi veya kurumun elektronik ortamdaki bağlantı bilgisi
- Bir kimsenin sık olarak gittiği yer
- Hedef gösterilen yer
- Toplanılan, bir araya gelinen yer
ozellikler:
- - isim
- - isim
  - ''
- - isim
  - ''
  - mecaz
- - isim
  - ''
  - mecaz
  - mecaz
- - isim
  - ''
  - mecaz
  - mecaz
  - mecaz
ornekler:
- - ''
- - ''
- - ''
- - ''
- - Genç Osmanlılar adındaki bu cemiyet, yönetime karşı olanların ortak adresiydi.
orneklerkaynak:
- - ''
- - ''
- - ''
- - ''
- - Uğur Kavas
---
