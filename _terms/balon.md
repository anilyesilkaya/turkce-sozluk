---
layout: term
title: balon
slug: balon
letter: B
lisan: Fransızca ballon
anlamlar:
- Isıtılmış hava veya havadan daha hafif bir gazla doldurulan, atmosferde uçabilen, küre biçiminde araç
- Hava veya gazla doldurulmuş, kauçuktan yapılan çocuk oyuncağı
- Karnı yuvarlak ve şişkin, boynu dar cam kap
- Karikatürde içi sözle doldurulan yuvarlak
- Geriden gelen rüzgârdan yararlanmak amacıyla yatın ana direği üzerine çekilen üç köşeli, hafif yelken
- ► palavra
ozellikler:
- - isim
- - isim
  - ''
- - isim
  - ''
  - ''
- - isim
  - ''
  - ''
  - ''
- - isim
  - ''
  - ''
  - ''
  - denizcilik
- - isim
  - ''
  - ''
  - ''
  - denizcilik
  - mecaz
ornekler:
- - Kendi icadı olan sıcak havayla doldurulmuş balonuyla Çorlu taraflarına inmiş ve bir sonraki yıl iki uçuş daha...
- - Hatırlattı bana, bir bayram sabahı / Gökyüzüne kaçırdığım balonuma bakıp ağlayışımı
- - ''
- - ''
- - ''
- - Son cereyan (gülümsedi ve tavana doğru bakarak) hesapla kitapla ‘Genç Kalemler’in Selanik'ten salıverdiği balondur.
orneklerkaynak:
- - Süleyman Faruk Göncüoğlu
- - Orhan Veli Kanık
- - ''
- - ''
- - ''
- - Ruşen Eşref Ünaydın
---
