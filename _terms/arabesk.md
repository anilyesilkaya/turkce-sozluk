---
layout: term
title: arabesk
slug: arabesk
lisan: Fransızca arabesque
anlamlar:
- Köyden kente göç eden toplulukların geleneksel yaşam tarzından çağdaş yaşam tarzına geçiş evresini yansıtan (kültür, sanat vb.)
- Arap müziğine ait icra anlayışının Türk müziğine uygulanmasıyla oluşmuş, söz ağırlıklı (müzik)
- ► girişik bezeme
- Bu tarz süslemelerle bezenmiş olan
ozellikler:
- - sıfat
- - sıfat
  - müzik
- - sıfat
  - müzik
  - mimarlık
- - sıfat
  - müzik
  - mimarlık
  - mimarlık
ornekler:
- - ''
- - Kahvehanenin kasetçalarında yanık sesli kadın arabesk bir şarkı söylüyordu, içerisi pek kalabalık değildi.
- - ''
- - Çiçek demetleri, frizler ve kordonlar, arabesk levhalar bu cephelerde bazen yazıya pek az yer bırakır.
orneklerkaynak:
- - ''
- - Ahmet Ümit
- - ''
- - Ahmet Hamdi Tanpınar
---
