---
layout: term
title: çanak
slug: canak
letter: Ç
lisan: Türkçe
anlamlar:
- Toprak, metal vb. bir maddeden yapılmış yayvan, çukurca kap
- göz çukuru
- Çiçeğin en dışında bulunan yeşil yaprakların tümü
- Çevresine göre alçakta bulunan, derinliği genişliğinden az olan arazi
- Göstermelik, yalan yanlış, önceden belirlenmiş sonucu almaya yönelik
ozellikler:
- - isim
- - isim
  - ''
- - isim
  - ''
  - bitki bilimi
- - isim
  - ''
  - bitki bilimi
  - coğrafya
- - isim
  - ''
  - bitki bilimi
  - coğrafya
  - sıfat
  - mecaz
ornekler:
- - O beni hiçbir sebebi yokken günlerce bir çanak süt için yalvarttı, aşağıladı.
- - Kanlı çanaklarından fırlayan iri parlak gözleri, pek korkunç bakıyordu.
- - ''
- - ''
- - O sayfaları hazırlayanlar karşımızdaki cephenin dolduruşuyla bir çanak anket düzenlediler.
orneklerkaynak:
- - Yusuf Kemal
- - Ömer Seyfettin
- - ''
- - ''
- - Refik Erduran
---
