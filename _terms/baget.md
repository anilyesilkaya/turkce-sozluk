---
layout: term
title: baget
slug: baget
letter: B
lisan: Fransızca baguette
anlamlar:
- Bateri çalmaya yarayan ince, kısa çubuk
- Tıraşlanmış, dikdörtgen biçiminde değerli taş
- Düşük gramajlı ince, uzun ekmek
- Tavuk, piliç vb. kanatlılarda but ile paça arasında kalan etli bölüm
- Çorabı gergin tutmak ve düşmesini engellemek için kullanılan sert lastik
ozellikler:
- - isim
- - isim
  - ''
- - isim
  - ''
  - ''
- - isim
  - ''
  - ''
  - ''
- - isim
  - ''
  - ''
  - ''
  - ''
ornekler:
- - Oturanların ortasında siyah pantolon giymiş olan kişi bando şefi, elinde bir baget tutuyor.
- - ''
- - Sıcak baget ekmeğinin içinde eriyen tereyağı o kadar iştah açıcı bir koku saçıyordu ki çocuklar tok oldukları hâlde onlar da kendilerine o sandviçlerden ısmarladılar.
- - ''
- - ''
orneklerkaynak:
- - ''
- - ''
- - Vivet Çavlı
- - ''
- - ''
---
